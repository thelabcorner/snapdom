...
describe('identity share — the gates, in pixels', () => {
  ...
  it('the focused element keeps its focus styling', async () => {
    ...
    // SnapDIFF compares the WHOLE production capture against the conservative no-share oracle.
    // This is stronger than the former one-pixel test: a routing bug anywhere on either twin is
    // visible, while SnapDIFF's default AA classifier keeps engine-specific edge rasterization
    // from masquerading as a semantic failure.
    ...
    // Keep an independent semantic sanity check so two identically-wrong captures cannot make
    // the differential oracle green. The fixture is deliberately text-free: the old button
    // sampled the centered "a" glyph, where WebKit rendered 51,0,0 over a correct red surface.
