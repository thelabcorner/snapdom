...
const baseMod = fs.readFileSync(path.join(ROOT, BASE_REL))
const candMod = fs.readFileSync(path.join(ROOT, CAND_REL))
...
const report={generatedAt:new Date().toISOString(),base:{path:BASE_REL,sha256:sha(baseMod)},candidate:{path:CAND_REL,sha256:sha(candMod)},mode:PARITY_ONLY?'parity-only':'timed',n:N,batch:BATCH,warmup:WARM,bootstrap:BOOT,fixtures,layouts:L};const outPath=path.join(OUT,`pwh1-bundle-${PARITY_ONLY?'parity':QUICK?'quick':'full'}.json`);fs.writeFileSync(outPath,JSON.stringify(report,null,2))
console.log(`PWH1 bundle base=${sha(baseMod).slice(0,12)} cand=${sha(candMod).slice(0,12)} ${PARITY_ONLY?'PARITY-ONLY':`N=${N} batch=${BATCH}`}`);for(const[f,x]of Object.entries(fixtures)){if(PARITY_ONLY)console.log(`${f.padEnd(28)} parity=${x.parity?'PASS':'FAIL'}`);else console.log(`${f.padEnd(28)} parity=${x.parity?'PASS':'FAIL'} effect ${x.candidate.pct.toFixed(1)}% CI[${x.candidate.ci95.map(v=>v.toFixed(1)).join(', ')}] null ${x.control.pct.toFixed(1)}% CI[${x.control.ci95.map(v=>v.toFixed(1)).join(', ')}] CoV=${(x.maxCov*100).toFixed(1)}% claim=${x.claimable?'PASS':x.regression?'REGRESSION':'INCONCLUSIVE'}`)}
