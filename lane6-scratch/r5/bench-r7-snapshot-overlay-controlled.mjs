#!/usr/bin/env node
// R7 decision-quality experiment: shared-snapshot overlay representation on the standing
// 400-card fixtures. Same bundle on both arms; only the internal reuse counterfactual differs.
// Four fresh physical layouts remove module-slot/JIT position bias and historical/historical
// crossover is the zero-effect control. Raw samples are retained; no outlier deletion.
...
const CAND_REL = arg('candidate', 'worktrees/snapdom-v3-r7-snapshot-overlay/dist/snapdom.mjs')
...
const FIXTURES = ONLY
  ...
  : DEFAULT_FIXTURES
const BASE = { ...FIXTURE_OPTIONS, __styleShareSnapshotOverlay: false }
const OPT = { ...FIXTURE_OPTIONS, __styleShareSnapshotOverlay: true }
...
const outPath = path.join(OUT, 'r7-snapshot-overlay-controlled.json')
fs.writeFileSync(outPath, JSON.stringify(report, null, 2))
console.log(`R7 snapshot overlay controlled ${sha.slice(0, 12)} ${ENGINE} N=${N} batch=${BATCH}`)
...
console.log('artifact lane6-scratch/r5/results/r7-snapshot-overlay-controlled.json')
