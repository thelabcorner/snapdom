#!/usr/bin/env node
// R7-P deterministic 2x2 + high-entropy routing probe. No wall-time claims.
...
const C = path.join(ROOT, 'worktrees/snapdom-v3-r7-pseudo-overlay')
const plugin = {
  name: 'r7p-factorial-probe',
  setup(b) {
...
const reset = () => ({ getStyleKey: 0, keyHit: 0, keyMiss: 0, compactSig: 0, fullSig: 0, cacheEligible: 0, cacheBypass: 0 })
try {
  await page.goto(`http://127.0.0.1:${server.address().port}`)
  await page.waitForFunction(() => window.__ready)
  const run = async (name, overlay, keyCache, mode = 'repeated') => page.evaluate(async ({ name, overlay, keyCache, mode }) => {
    globalThis.__r7pProbe = { getStyleKey: 0, keyHit: 0, keyMiss: 0, compactSig: 0, fullSig: 0, cacheEligible: 0, cacheBypass: 0 }
    ...
    if (mode === 'pairUnique' || mode === 'tripleUnique' || mode === 'breakerTail') {
      const color = (i, salt) => 'rgb(' + ((i * 47 + salt) % 256) + ',' + ((i * 83 + salt * 3) % 256) + ',' + ((i * 131 + salt * 7) % 256) + ')'
      const uniqueCardinality = mode === 'breakerTail' ? 18 : mode === 'tripleUnique' ? 120 : 200
    ...
    for (let i = 0; i < nodes; i++) {
      ...
      e.className = ['pairUnique', 'tripleUnique', 'pairSame', 'tripleSame', 'quadSame', 'fiveSame'].includes(mode)
        ? 'row g' + (i % cardinality)
        : 'row'
  ...
  for (const r of repeated) {
    console.log(r.name, { rawEqual: r.raw === repeatedBase, getStyleKey: r.getStyleKey, cacheEligible: r.cacheEligible, cacheBypass: r.cacheBypass, keyHit: r.keyHit, keyMiss: r.keyMiss, compactSig: r.compactSig, fullSig: r.fullSig })
  }
  ...
  for (const r of [entropyHistorical, entropyCombined]) {
    console.log(r.name, { rawEqual: r.raw === entropyHistorical.raw, getStyleKey: r.getStyleKey, cacheEligible: r.cacheEligible, cacheBypass: r.cacheBypass, keyHit: r.keyHit, keyMiss: r.keyMiss, compactSig: r.compactSig, fullSig: r.fullSig })
  }
  ...
  for (const r of [pairHistorical, pairCombined]) {
    console.log(r.name, { rawEqual: r.raw === pairHistorical.raw, getStyleKey: r.getStyleKey, cacheEligible: r.cacheEligible, cacheBypass: r.cacheBypass, keyHit: r.keyHit, keyMiss: r.keyMiss, compactSig: r.compactSig, fullSig: r.fullSig })
  }
  ...
  for (const r of [tripleUniqueHistorical, tripleUniqueCombined]) {
    console.log(r.name, { rawEqual: r.raw === tripleUniqueHistorical.raw, getStyleKey: r.getStyleKey, cacheEligible: r.cacheEligible, cacheBypass: r.cacheBypass, keyHit: r.keyHit, keyMiss: r.keyMiss, compactSig: r.compactSig, fullSig: r.fullSig })
  }
  ...
  for (const mode of ['pairSame', 'tripleSame', 'quadSame', 'fiveSame']) {
    ...
    for (const r of [h, c]) {
      console.log(r.name, { rawEqual: r.raw === h.raw, getStyleKey: r.getStyleKey, cacheEligible: r.cacheEligible, cacheBypass: r.cacheBypass, keyHit: r.keyHit, keyMiss: r.keyMiss, compactSig: r.compactSig, fullSig: r.fullSig })
    }
  }
  ...
  for (const r of [breakerHistorical, breakerCombined]) {
    console.log(r.name, { rawEqual: r.raw === breakerHistorical.raw, getStyleKey: r.getStyleKey, cacheEligible: r.cacheEligible, cacheBypass: r.cacheBypass, keyHit: r.keyHit, keyMiss: r.keyMiss, compactSig: r.compactSig, fullSig: r.fullSig })
  }
