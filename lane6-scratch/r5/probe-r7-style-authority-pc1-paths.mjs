#!/usr/bin/env node
// R7-PC1 mechanism counts on top of the current style-authority integration scout.
// In-memory instrumentation only; no timing evidence.
...
const CANDIDATE = path.join(ROOT, 'worktrees/snapdom-v3-r7-style-authority-integration')
const plugin = {
  name: 'r7-style-authority-pc1-paths',
  setup(ctx) {
    ...
    try {
      ...
      const out = await page.evaluate(async ({ pseudoCss }) => {
        ...
        try {
          ...
          return globalThis.__counterProbe
