#!/usr/bin/env node
// R7-SA4 deterministic same-build counterfactual for backdrop-pass style reuse.
...
const REL = 'worktrees/snapdom-v3-r7-backdrop-style-reuse/dist/snapdom.mjs'
...
const fixtures = ['light-20cards', 'cards400-safe', 'cards400-neutral-unsafe', 'cards400-non-neutral', 'asset-heavy', 'entropy-400', 'backdrop-120']
...
try {
  ...
  for (const fixture of fixtures) {
    const run = async (reuse) => page.evaluate(async ({ fixture, opts, reuse }) => {
      ...
      } else if (fixture === 'backdrop-120') {
        style = document.createElement('style')
        style.textContent = '.bd-root{width:900px}.bd{display:block;width:80px;height:12px;background:rgba(255,255,255,.2)}.bd:nth-child(6n){backdrop-filter:blur(2px)}'
        document.head.appendChild(style)
        el = document.createElement('div'); el.className = 'bd-root'
        for (let i = 0; i < 120; i++) {
          const r = document.createElement('div'); r.className = 'bd'; r.textContent = 'row ' + i; el.appendChild(r)
        }
        document.body.appendChild(el)
      } else {
      ...
      try {
        ...
        if (fixture === 'entropy-400' || fixture === 'backdrop-120') { el.remove(); style?.remove() }
