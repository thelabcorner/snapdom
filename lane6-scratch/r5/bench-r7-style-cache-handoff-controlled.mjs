#!/usr/bin/env node
// R7 decision-quality experiment: SA1/SA2 phase-shared live-style handoff 2x2 factorial.
// Same bundle on all arms; only the internal seed counterfactuals differ:
//   00 historical (both seeds false), 01 SA1 only, 10 SA2 only, 11 combined.
...
const CAND_REL = arg('candidate', 'worktrees/snapdom-v3-r7-style-cache-handoff/dist/snapdom.mjs')
...
const A00 = { ...FIXTURE_OPTIONS, __lineClampStyleSeed: false, __contentVisibilityStyleSeed: false }
const A01 = { ...FIXTURE_OPTIONS, __lineClampStyleSeed: false, __contentVisibilityStyleSeed: true }
const A10 = { ...FIXTURE_OPTIONS, __lineClampStyleSeed: true, __contentVisibilityStyleSeed: false }
const A11 = { ...FIXTURE_OPTIONS, __lineClampStyleSeed: true, __contentVisibilityStyleSeed: true }

// [pairId, slot1Opts, slot2Opts, seed]: effect = slot2 vs slot1.
const PAIRS = [
  ['sa1-main', A00, A01, 0x5a01],
  ['sa2-only', A00, A10, 0x5a02],
  ['sa2-cond', A01, A11, 0x5a03],
  ['sa1-cond', A10, A11, 0x5a04],
  ['combined', A00, A11, 0x5a05],
]
...
try {
  ...
  layouts['null-forward'] = await layout('null-forward', A00, A00)
  layouts['null-reverse'] = await layout('null-reverse', A00, A00)
} finally {
  await browser.close()
...
const report = {
  ...
  method: 'same bundle 2x2 option-only; fresh-page slot crossover per pair; symmetric warmup; paired log ratios; full-copy/full-copy null; seeded bootstrap; no outlier deletion',
  fixtures,
  layouts,
}
const outPath = path.join(OUT, 'r7-style-cache-handoff-controlled.json')
fs.writeFileSync(outPath, JSON.stringify(report, null, 2))
console.log(`R7 style-cache-handoff 2x2 controlled ${sha.slice(0, 12)} ${ENGINE} N=${N} batch=${BATCH}`)
...
console.log('artifact lane6-scratch/r5/results/r7-style-cache-handoff-controlled.json')
