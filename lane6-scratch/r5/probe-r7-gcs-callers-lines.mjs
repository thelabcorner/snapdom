#!/usr/bin/env node
// R7 diagnostic: aggregate getComputedStyle caller chains on an unminified corrected-parent build.
// Deterministic attribution only; instrumentation is intentionally invasive and supplies no timing evidence.
...
const entry=path.join(ROOT,'worktrees/snapdom-v3-clean-456/src/index.js')
...
try{
 ...
 for(const fixture of ['cards400-safe','asset-heavy']){
   const rows=await page.evaluate(async ({fixture,opts})=>{
     const orig=window.getComputedStyle;const counts=new Map()
     window.getComputedStyle=function(...args){
       const chain=(new Error().stack||'').split('\n').slice(2,6).map(x=>x.trim().replace(/https?:\/\/[^/]+\/m\.mjs:\d+:\d+/,'m.mjs')).join(' <- ')
       counts.set(chain,(counts.get(chain)||0)+1)
       return orig.apply(this,args)
     }
     const el=window.__fx.build(fixture)
     try{await window.__m.snapdom.toRaw(el,{...opts,cache:'disabled'})}finally{window.__fx.cleanup(el);window.getComputedStyle=orig}
     return [...counts].map(([chain,count])=>({chain,count})).sort((a,b)=>b.count-a.count).slice(0,30)
   },{fixture,opts:FIXTURE_OPTIONS})
   console.log(`\n${fixture}`);for(const r of rows)console.log(String(r.count).padStart(6),r.chain)
 }
 await page.close()
