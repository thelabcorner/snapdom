...
// E2 defers setup until the fifth observed occurrence. Past reuse is evidence, not a promise
// of future hits: five-only identities remain an explicit protected timing adversary.
const PSEUDO_REUSE_MIN_OCCURRENCES = 5
 ...
 * Unique/pair/triple/quad-only identities therefore stay on the historical key path unless enough
...
export function pseudoStyleKeyForSnapshot(snap, sizedByContent, isFlexItem, session, useCache = true) {
  if (!useCache || !session || !__sharedPseudoSnapshots.has(snap) || session.__pseudoStyleKeyCacheDisabled) {
...
export function pseudoSnapshotFor(source, pseudo, style, session, options) {
  ...
  if (rec) {
    ...
    if (!overlayEnabled && !useKeyCache) {
      // Same-bundle historical arm: preserve the R7 copy/list/signature order as well as values.
      const snap = { ...rec.snap }
    ...
    // E1 spread #2/#3 and first paid setup at #4. E2 also spreads #4; the seed counts as #1.
    const reuseReady = ++rec.seen >= PSEUDO_REUSE_MIN_OCCURRENCES
    ...
    if (cacheEligible && session?.__pseudoStyleKeyCacheDisabled) {
      // The global miss breaker protects heterogeneous admitted identities from repeated
  ...
  snaps.set(key, { snap, rr: null, sig: null, seen: 1, h: 'height' in snap, b: 'block-size' in snap })
  return snap
}
...
function getSnapshot(el, preStyle = null, options = {}, shareInfo = null) {
  ...
  if (shared) {
    // Identity hit: reuse the shared full read, then re-read only the layout-varying props on
    ...
    const insetGate = options?.__styleShareInsetValueGate !== false
    let rrList
    if (useOverlay) {
      ...
      rrList = shared.rr || shareLists(shared, el, insetGate, false)
      ...
      snap = { ...shared.snap }
      rrList = shared.rr || shareLists(shared, el, insetGate)
    }
...
export function inlineAllStyles(source, clone, sessionOrCtx, opts) {
  ...
  if (flexItem) {
    ...
    // same-capture value when present; excluded/missing values take the historical live read.
