...
/**
 * Inherited dependency context contributed by `node` and its ancestors to descendants.
 *
 * R3 originally walked the complete ancestor chain for every element. Once inherited rule
 * dependencies became selector-proven, that would also re-run the same matches() calls for
 * every descendant. Cache the cumulative proof per style scan instead: a mutation/state
 * change bumps the style epoch, scanFor() returns a new scan record, and
 * elementUniverseStateFor() therefore creates a fresh WeakMap.
 *
 * `null` is the conservative answer when the chain cannot be proven safely.
 */
function inheritedElementContextFor(node, scan, st, universe) {
  ...
  // Find the nearest cached ancestor, then resolve the uncached suffix top-down. The explicit
  // stack avoids recursion on pathological DOM depth. Matching the old 1024-level guard is
  // conservative: unlike the old loop, exceeding it now fails closed instead of silently
  // ignoring more ancestors.
  const pending = []
  let cur = node
...
function elementUniverseFor(el, style, options, universe, backgroundState = null) {
  ...
  // Resolve inherited/ancestor dependencies once per style epoch and reuse them for every
  // descendant instead of re-walking/re-matching the same chain.
  ...
  // Current-element conservative boundaries and UA/default differences remain local.
